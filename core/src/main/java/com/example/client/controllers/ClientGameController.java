...
//    }
    ...
    public static Result buyAnimal(Player buyer, AnimalType animalType, String name) {
        ...
        if(placed) {
            ClientApp.currentGame.getDateAndTime().addObserver(animal);
    ...
    public static Result collectProduce(Animal animal) {
        if(animal.getCurrentProduct() == null) {
            ...
            return new Result(false, "you have to use milk pail to collect milk.");
        }
        else if(animal.getAnimalType().equals(AnimalType.SHEEP)) {
            return new Result(false, "you have to use shear to collect wool.");
        }
    ...
    public static Result sellAnimal(Animal animal) {
        ...
        ClientApp.currentGame.getDateAndTime().removeObserver(animal);
    ...
    public static Result fishing(String material) {
        ...
        for(int row = getCurrentPlayer().getPosition().y - 1; row <= getCurrentPlayer().getPosition().y + 1; row++) {
            for(int col = getCurrentPlayer().getPosition().x - 1; col <= getCurrentPlayer().getPosition().x + 1; col++) {
                if(ClientApp.currentGame.getTile(col, row).getArea() instanceof Lake) {
                    lake = (Lake) ClientApp.currentGame.getTile(col, row).getArea();
                    break;
                }
            }
        }
        ...
        if(lake == null) {
            ...
            return new Result(true, fishingPole.use(lake, getCurrentPlayer(), ClientApp.currentGame.getWeather().getCurrentWeather()));
        }
    }
    ...
    public static Result showStoreProducts() {
        ...
        if(!store.isOpen(ClientApp.currentGame.getDateAndTime().getHour())) {
    ...
    public static Result sellProduct(String productName, int count) {
        ...
        for(int row = getCurrentPlayer().getPosition().y - 1; row <= getCurrentPlayer().getPosition().y + 1; row++) {
            for(int col = getCurrentPlayer().getPosition().x - 1; col <= getCurrentPlayer().getPosition().x + 1; col++) {
                if(row >= 0 && col >= 0) {
                    Tile tile = ClientApp.currentGame.getTile(col, row);
    ...
    public static Result talkFriendship(String username, String message) {
        Player receiver = ClientApp.currentGame.getPlayerByUsername(username);
        if(receiver == null) {
        ...
        Tile playerTile = ClientApp.currentGame.getTile(getCurrentPlayer().getPosition());
        Tile recieverTile = ClientApp.currentGame.getTile(receiver.getPosition());
        if(!playerTile.isAdjacent(recieverTile)) {
            return new Result(false, "you have to be next to a player to talk to them.");
        }

        PlayerFriendship friendship = ClientApp.currentGame.getFriendshipByPlayers(getCurrentPlayer(), receiver);
    ...
    public static Result talkHistory(String username) {
        ...
        PlayerFriendship friendship = ClientApp.currentGame.getFriendshipByPlayers(getCurrentPlayer(), target);
        StringBuilder sb = new StringBuilder();
    ...
    public static Result giftList(String username) {
        ...
        PlayerFriendship friendship = ClientApp.currentGame.getFriendshipByPlayers(sender, getCurrentPlayer());
        StringBuilder sb = new StringBuilder();
        sb.append("gift list: \n");
    ...
    public static Result marry(String username) {
        ...
        Tile playerTile = ClientApp.currentGame.getTile(getCurrentPlayer().getPosition());
        Tile targetTile = ClientApp.currentGame.getTile(target.getPosition());
    ...
    public static Result fertilize(String fertilizerName, Position position) {
        Tile goalTile = ClientApp.currentGame.getTile(position.x,position.y);
    ...
    public static Result plantMixedSeed(int dx,int dy) {
        ...
        CropSeeds randomSeed = MixedSeedCrop.getRandomSeed(ClientApp.currentGame.getDateAndTime().getSeason());
    ...
    public static Result showPlant(int x, int y) {
        ...
        Tile tile = ClientApp.currentGame.getMap().getTile(position);
    ...
    public static Result PlaceItem(String itemName, int x , int y) {
        ...
        Tile tile = ClientApp.currentGame.getTile(getCurrentPlayer().getPosition().x + x, getCurrentPlayer().getPosition().y + y);
    ...
    public static Result UseArtisan(String artisanName , String itemName) {
        ...
        if (artisanItem.getName().equals("honey")){
            Game game = ClientApp.currentGame;
