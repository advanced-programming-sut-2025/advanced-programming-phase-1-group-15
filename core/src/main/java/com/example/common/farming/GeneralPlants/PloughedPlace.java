...
import com.example.common.weather.WeatherObserver;
...
public class PloughedPlace implements TimeObserver , Tilable, WeatherObserver {
    ...
    public DateAndTime lastUpdate(){
        return ClientApp.currentGame.getDateAndTime();
    }
            ...
            for (List<Position> group : groups) {
                if (canBecomeGiant(group)) {
                    ...
                    for (Position p : group) {
                        tiles.add(ClientApp.currentGame.getTile(p));
                    }
    ...
    public PloughedPlace() {
        ClientApp.currentGame.getDateAndTime().addObserver(this);
        updateDay();
    }

    public void updateDay(){
        lastUpdateOfDay = ClientApp.currentGame.getDateAndTime().getDay();
    }
    ...
    private boolean canBecomeGiant(List<Position> positions) {
        ...
        Tile firstTile = ClientApp.currentGame.getTile(first);
        ...
        for (Position p : positions) {
            ...
            Tile t = ClientApp.currentGame.getTile(p);
    ...
    private Crops getCropTypeOfPos(Position pos){
        Tile tileOfPos = ClientApp.currentGame.getTile(pos);
    ...
    public Sprite getSprite() {
        if(currentState instanceof PloughedState){
            ...
            if(harvestable instanceof Tree){
                ...
                Sprite res =  sprites.get(Math.min(sprites.size()-1, (int)((1.5)*(((SeededState)currentState).getGrowthLevel()))));
                return res;
                ...
                Sprite res =  sprites.get(Math.min(sprites.size()-1, (int)((1.2)*(((SeededState)currentState).getGrowthLevel()))));
                return res;
            }
        }
        else{
