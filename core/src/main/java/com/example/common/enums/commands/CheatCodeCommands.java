...
public enum CheatCodeCommands implements Commands {
    ...
    SET_FARMING_ABILITY("\\s*set\\s+farming\\s+(?<username>\\S+)\\s+(?<amount>\\d+)\\s*$"),
    SET_FISHING_ABILITY("\\s*set\\s+fishing\\s+(?<username>\\S+)\\s+(?<amount>\\d+)\\s*$"),
    SET_FORAGING_ABILITY("\\s*set\\s+foraging\\s+(?<username>\\S+)\\s+(?<amount>\\d+)\\s*$"),
    SET_MINING_ABILITY("\\s*set\\s+mining\\s+(?<username>\\S+)\\s+(?<amount>\\d+)\\s*$"),
